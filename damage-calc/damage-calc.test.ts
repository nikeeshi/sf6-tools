...
import { calculateCombo, type MoveInput } from './damage-calc.ts';
...
// 6段目のHPは「MP>HP」というターゲットコンボ固有の技で、単発のst.HP(630)とは別物。
// 本人確認済みの実値は360。7段目のMKは236MKからのスライディング派生で
// 独立ソースが無いため逆算(720。720*0.29=208.8→切り捨てで208と一致する範囲{718,719,720}のうち採用)。
test('キンバリー実測コンボ: MK(CH)>CR>2HP 236LK>LP 2MP 2MP MP>HP 236MK>MK', () => {
  const moves: MoveInput[] = [
    ...
    { name: 'HP(MP>HPのターゲットコンボ固有値)', baseDamage: 360, comboScaling: 10, multipliers: { driveRush: true } },
