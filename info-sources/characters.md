...
- ~~**pachi-mea**~~ — **除外した**(匿名テンプレ型。README の除外リスト)。番号は記録として残すが使わない
...
**pachi-mea は除外した。** 全31キャラが均等に揃っていて各ページに「キャラ相性」「キャラ対策」の節がある、
という**構造だけを見て採用と書き、本文を1ページも読んでいなかった**。書き手が名乗っていない
匿名テンプレ型なので落とした(README の除外リスト)。**受け側を全キャラ分埋める手段は現在ない。**
代替候補はかくぶろの `anti-` 記事と SF6Wiki。
